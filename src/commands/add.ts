...
export const command = new SlashCommandBuilder()
  .setName('add')
  .setDescription('Aggiungi il ruolo SS Team ad un utente')
...
export async function execute(interaction: CommandInteraction, config: any) {
  ...
  try {
    ...
    await interaction.reply({
      content: `${gestore} ha dato il ruolo <@&1291366009374117929> a <@${member.user.id}>`,
      ephemeral: true
    });
  } catch (error) {
    console.error(error);
    return interaction.reply({ content: "Errore nell'aggiungere il ruolo. Riprova più tardi.", ephemeral: true });
  }
}
